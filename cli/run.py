...
from cli.utils.mode import auto_detect_user_mode, auto_detect_mode_for_run
...
def start_program():
    ...
    if is_systemd_available():
        service_running = is_service_running("encryptsync")
    ...
    if not cfg.exists():
        logger.error(f"[run] Config file not found at {cfg}. "
                     f"Use `encryptsyncctl edit --user` (user mode) ou `encryptsyncctl edit` (system).")
        raise SystemExit(1)
